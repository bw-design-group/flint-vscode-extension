...
export class DesignerConnectionManager implements IServiceLifecycle {
    ...
    private wslProxy: WslProxy | null = null;
    ...
    private onRecordingEventCallbacks: Array<(batch: RecordingEventBatch) => void> = [];

    private onRecordingCompleteCallbacks: Array<(data: RecordingCompleteData) => void> = [];
    ...
    /**
     * Registers a callback for recording event batches from the Designer
     */
    onRecordingEvent(callback: (batch: RecordingEventBatch) => void): void {
        this.onRecordingEventCallbacks.push(callback);
    }

    /**
     * Removes a recording event callback
     */
    offRecordingEvent(callback: (batch: RecordingEventBatch) => void): void {
        const index = this.onRecordingEventCallbacks.indexOf(callback);
        if (index >= 0) {
            this.onRecordingEventCallbacks.splice(index, 1);
        }
    }

    /**
     * Registers a callback for recording completion notifications
     */
    ...
    /**
     * Removes a recording completion callback
     */
    offRecordingComplete(callback: (data: RecordingCompleteData) => void): void {
        const index = this.onRecordingCompleteCallbacks.indexOf(callback);
        if (index >= 0) {
            this.onRecordingCompleteCallbacks.splice(index, 1);
        }
    }
    ...
    async dispose(): Promise<void> {
        ...
        this.onRecordingEventCallbacks = [];
        this.onRecordingCompleteCallbacks = [];
    }
    ...
    /**
     * Gets the WebSocket constructor class (for sharing with secondary connection managers)
     */
    getWebSocketClass(): WebSocketConstructor | null {
    ...
    /**
     * Gets the capabilities reported by the connected Designer module
     */
    ...
    async connect(designer: DesignerInstance): Promise<void> {
        ...
        try {
            await this.connectToPort(designer, designer.port);
        } catch (directError: unknown) {
            // On WSL, fall back to proxy if direct connection fails
            if (isWSL()) {
                try {
                    const { createWslProxy } = await import('@/utils/wslProxy');
                    this.wslProxy = await createWslProxy(designer.port);
                    console.log(`[WSL] Created proxy on port ${this.wslProxy.localPort} -> ${designer.port}`);
                    await this.connectToPort(designer, this.wslProxy.localPort);
    ...
    /**
     * Establishes a WebSocket connection to a specific port
     */
    private connectToPort(designer: DesignerInstance, port: number): Promise<void> {
     ...
     */
    async perspectiveProfileView(
        sessionId: string,
        pageId: string,
        viewInstanceId: string
    ): Promise<ViewProfileResult> {
        return this.sendRequest('perspective.profileView', { sessionId, pageId, viewInstanceId });
    }

    // ==================== Perspective Recording Methods ====================

    /**
     * Starts a binding recording session for a live Perspective view
     */
    async perspectiveStartRecording(params: StartRecordingParams): Promise<StartRecordingResult> {
        return this.sendRequest('perspective.startRecording', {
            sessionId: params.sessionId,
            pageId: params.pageId,
            viewInstanceId: params.viewInstanceId,
            pollIntervalMs: params.pollIntervalMs ?? 50,
            maxDurationMs: params.maxDurationMs ?? 120000,
            autoStopOnAllResolved: params.autoStopOnAllResolved ?? false,
            autoStopDelayMs: params.autoStopDelayMs ?? 2000
        });
    }
     ...
     */
    async perspectiveStopRecording(recordingId: string): Promise<StopRecordingResult> {
        return this.sendRequest('perspective.stopRecording', { recordingId });
    }
    ...
    private handleNotification(notification: JsonRpcNotification): void {
        ...
        // Handle recording event batches
        if (notification.method === 'perspective.recordingEvent') {
            const batch = notification.params as RecordingEventBatch;
            for (const callback of this.onRecordingEventCallbacks) {
                try {
                    callback(batch);
                } catch (error) {
        ...
        if (notification.method === 'perspective.recordingComplete') {
            const data = notification.params as RecordingCompleteData;
            for (const callback of this.onRecordingCompleteCallbacks) {
                try {
                    callback(data);
                } catch (error) {
