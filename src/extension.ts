...
import { AnalyzeViewCommand } from '@/commands/perspective/AnalyzeViewCommand';
import { ProfilePageCommand } from '@/commands/perspective/ProfilePageCommand';
import { ProfileViewCommand } from '@/commands/perspective/ProfileViewCommand';
...
import { CopyTagPathCommand } from '@/commands/tags/CopyTagPathCommand';
import { CreateTagCommand } from '@/commands/tags/CreateTagCommand';
import { DeleteTagCommand } from '@/commands/tags/DeleteTagCommand';
import { GetTagConfigCommand } from '@/commands/tags/GetTagConfigCommand';
import { ReadTagCommand } from '@/commands/tags/ReadTagCommand';
import { RefreshTagBrowserCommand } from '@/commands/tags/RefreshTagBrowserCommand';
import { WriteTagCommand } from '@/commands/tags/WriteTagCommand';
...
import { LanguageServerService } from '@/services/languageServer/LanguageServerService';
...
async function initializeServices(
    ...
    // Initialize Tag Browser service (depends on DesignerBridgeService)
    const tagBrowserService = new TagBrowserService(serviceContainer);
    serviceContainer.register('TagBrowserService', tagBrowserService);
    await tagBrowserService.initialize();
    await tagBrowserService.start();
    ...
    // Initialize Profiler webview service
    const profilerWebview = new ProfilerWebview(serviceContainer, context);
    serviceContainer.register('ProfilerWebview', profilerWebview);
    await profilerWebview.initialize();
    await profilerWebview.start();

    // Initialize Page Profiler webview service
    const pageProfilerWebview = new PageProfilerWebview(serviceContainer, context);
    serviceContainer.register('PageProfilerWebview', pageProfilerWebview);
    await pageProfilerWebview.initialize();
    await pageProfilerWebview.start();
    ...
    // Initialize the Flint language server (gateway-backed Jython intelligence via flint-lsp-proxy)
    const languageServerService = new LanguageServerService(serviceContainer);
    serviceContainer.register('LanguageServerService', languageServerService);
    await languageServerService.initialize();
    await languageServerService.start();
...
function registerCommands(commandRegistry: CommandRegistry, commandContext: any): void {
    try {
        ...
        // Perspective profiler commands
        commandRegistry.register(new AnalyzeViewCommand(commandContext));
        commandRegistry.register(new ProfileViewCommand(commandContext));
        commandRegistry.register(new ProfilePageCommand(commandContext));

        // Tag Browser commands
        commandRegistry.register(new ReadTagCommand(commandContext));
        commandRegistry.register(new WriteTagCommand(commandContext));
        commandRegistry.register(new GetTagConfigCommand(commandContext));
        commandRegistry.register(new CreateTagCommand(commandContext));
        commandRegistry.register(new DeleteTagCommand(commandContext));
        commandRegistry.register(new CopyTagPathCommand(commandContext));
        commandRegistry.register(new RefreshTagBrowserCommand(commandContext));
...
async function initializeUI(context: vscode.ExtensionContext, state: ExtensionState): Promise<void> {
    try {
        ...
        // Create tag browser tree view
        const tagBrowserService = state.serviceContainer.get<TagBrowserService>('TagBrowserService');
        const tagTreeDataProvider = new TagTreeDataProvider(tagBrowserService);
        await tagTreeDataProvider.initialize();
        await tagTreeDataProvider.start();
        state.serviceContainer.register('TagTreeDataProvider', tagTreeDataProvider);

        const tagTreeView = vscode.window.createTreeView('flintTagBrowser', {
            treeDataProvider: tagTreeDataProvider,
            showCollapseAll: true
        });

        tagTreeView.onDidChangeVisibility(e => {
            tagTreeDataProvider.handleVisibilityChange(e.visible);
        });

        // Refresh tag tree when Designer connection state changes
        const bridgeServiceForTags = state.serviceContainer.get<DesignerBridgeService>('DesignerBridgeService');
        bridgeServiceForTags.onConnectionStateChanged((connectionState: ConnectionState) => {
        ...
        // Register the legacy Designer-bridge Python completion provider only when the
        // gateway-backed language server is disabled — otherwise the LanguageClient owns
        // Python completion (and the two would produce duplicate items).
...
function setupFileWatchers(context: vscode.ExtensionContext, state: ExtensionState): void {
    ...
    const fileSaveSubscription = vscode.workspace.onDidSaveTextDocument(async document => {
        ...
        try {
            ...
            const hasAnyDesignerConnection = designerBridgeService?.hasAnyConnection() === true;

            // For config files, only trigger scan if connected to any designer
            if (isConfigFile && !hasAnyDesignerConnection) {
                return;
            }

            if (hasAnyDesignerConnection) {
                const scanResult = await designerBridgeService.scanAllDesigners();
                if (scanResult.totalScanned > 0) {
                    if (scanResult.allSucceeded) {
                        console.log(`[Flint] Designer project scan complete (${scanResult.totalScanned} Designer(s))`);
                    } else {
                        console.log(`[Flint] Designer project scan: some of ${scanResult.totalScanned} scans failed`);
                    }
                    // If primary succeeded, don't fall through to gateway scan
                    if (scanResult.primaryResult?.success) {
                        return;
                    }
                }
                // If primary scan failed or no primary, fall through to Gateway scan
                console.log('[Flint] Primary Designer scan unavailable, falling back to Gateway scan');
            }
