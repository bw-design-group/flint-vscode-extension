...
/**
 * Reads the API token value from a token file.
 ...
 * @returns The bare token value (e.g. `keyId:secret` for Ignition 8.3)
 * @throws Error if the file is missing or its format is invalid
 */
export async function readApiTokenValue(apiTokenFilePath: string, workspaceRoot?: string): Promise<string> {
    try {
...
async function getAuthHeaders(apiTokenFilePath: string, workspaceRoot?: string): Promise<Record<string, string>> {
    const token = await readApiTokenValue(apiTokenFilePath, workspaceRoot);
    return {
