...
export class DesignerStatusBarItem implements IServiceLifecycle {
    ...
    initialize(): Promise<void> {
        try {
            ...
            // Listen for secondary connection changes to update the display
            this.bridgeService.onSecondaryConnectionsChanged(() => {
                if (this.bridgeService?.getConnectionState() === ConnectionState.CONNECTED) {
                    this.updateDisplay(ConnectionState.CONNECTED, this.bridgeService.getConnectedDesigner());
                }
            });
    ...
    private updateConnectedDisplay(designer: DesignerInstance | null, matchResult: GatewayMatchResult | null): void {
        ...
        const secondaryCount = this.bridgeService?.getSecondaryConnections().getConnectionCount() ?? 0;
        ...
        // Show secondary connection count if any
        const secondarySuffix = secondaryCount > 0 ? ` (+${secondaryCount} more)` : '';
        this.statusBarItem.text = `${matchIcon} Designer: ${projectName}${secondarySuffix}`;
    ...
    private buildConnectedTooltip(designer: DesignerInstance | null, matchResult?: GatewayMatchResult | null): string {
        ...
        // Secondary connections info
        const secondaries = this.bridgeService?.getSecondaryConnections().getConnectedDesigners() ?? [];
        if (secondaries.length > 0) {
            lines.push('', `Secondary connections (${secondaries.length}):`);
            for (const secondary of secondaries) {
                lines.push(
                    `  ${secondary.project.name} @ ${secondary.gateway.host}:${secondary.gateway.port} (PID ${secondary.pid})`
                );
            }
        }

        lines.push('', 'Click to send a message to Designer');
